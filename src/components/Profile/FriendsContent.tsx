...
export default function FriendsContent() {
    ...
    const onAccept =async (id:any,friendId: number) => {
        try {
    ...
    const onIgnore = (friendId: number) => {
    };
            ...
            <div className='w-full max-w-[1230px] py-3 px-4 m-auto'>
                <div className='w-full rounded-[12px] bg-[var(--sections)] border border-white/5'>
                    <div className='w-full'>
                        ...
                        {requestData && requestData.map((item: any, index: any) => {
                            return (
                                <section key={item.id} className='w-full bg-[var(--sections)] border border-white/5 rounded-[12px]'>
                                    <section className='cursor-pointer flex items-start justify-between gap-4 p-4'>
                                        ...
                                        <div className='flex gap-4'>
                                            <button
                                                onClick={() => onAccept(item.id,item.user?.id)}
                                                className='bg-[var(--highlight)] text-white font-bold p-2 rounded-lg flex gap-1 items-center justify-center'>
                                                Accept
                                            </button>
                                            <button
                                                onClick={() => onIgnore(item.user?.id)}

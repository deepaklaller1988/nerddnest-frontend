                            ...
                            scheduledPosts.map((post) => (
                                <div key={post.id} className="w-full flex flex-col gap-3 rounded-lg bg-white/10 p-4">
                                    <section className="cursor-pointer flex items-center gap-2 pr-14 relative">
                                        ...
                                        <span className="w-full">
                                            ...
                                            <p className="text-[12px] text-white">
                                                <b className="font-semibold text-white">Schedule for:</b> {new Date(post.schedule_time).toLocaleString()} <GoGlobe className="fill-white inline-block" />
                                            </p>
                                        </span>
                                        <span className="flex absolute top-2 right-0 gap-3">
                                            <button type="button">
                                                <MdEdit size="16" className="hover:fill-white duration-[.5s]" />
                                            </button>
                                            <button type="button">
                                                <MdDeleteForever size="16" className="hover:fill-white duration-[.5s]" onClick={() => handleDeleteClick(post.id)} />
                                            </button>
                                        </span>
                                    </section>
                                    <p>{post.content}</p>
                                    {post.media_url && post.media_url.length > 0 && (
                                        <div className="mt-2">
                                            {post.media_url.map((media: string, index: number) => (
